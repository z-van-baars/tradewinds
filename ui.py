...
class MarketMenu(Menu):
    def __init__(self, screen, player, port):
        ...
        def sell_click():
            ...
            new_quantity_popup = QuantityMenu(screen, player, spice_name, spice_quantity)
            ...
            if amount_to_sell != 0:
                ...
                self.player.silver += amount_to_sell * 1
        ...
        def buy_click():
            ...
            if amount_to_buy != 0 and current_cargo + amount_to_buy <= self.player.ship.cargo_cap and amount_to_buy <= self.player.silver:
                ...
                self.player.silver -= amount_to_buy
    ...
    def render_decals(self, screen):
        ...
        for spice_name in self.display_cache["market visible items"]:
            ...
            screen.blit(spice_name_stamp, [x + 40 + 50,
                                           y + 50 + count * spacer])
        ...
        for spice_name in self.display_cache["cargo visible items"]:
            if self.player.ship.cargo[spice_name] > 0:
                ...
                screen.blit(spice_name_stamp, [x + 255 + 50,
                                               y + 50 + count * spacer])
...
class QuantityMenu(Menu):
    def __init__(self, screen, player, commodity, commodity_max):
        super().__init__(screen, player)
        self.background_pane = pygame.sprite.Sprite()
    ...
    def render_decals(self, screen):
        ...
        quantity_stamp = small_font.render(str(self.display_cache["commodity quantity"]), True, (255, 255, 255))
        left_margin = (self.background_pane.image.get_width() / 2) - (quantity_stamp.get_width() / 2)
        screen.blit(quantity_stamp,
                    [x + left_margin, y + 70])
