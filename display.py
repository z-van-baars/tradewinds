...
def update_display(game_state, selected_tile, display_parameters, mouse_pos, map_xy):
    ...
    player_screen_coordinates = (player_pixel_coordinates[0] + background_x_middle + (tile_width / 2),
    ...
    if game_state.player.ship.path:
        # print(len(game_state.player.ship.path.steps))
        shifted_pts_list = []
        for point in game_state.player.ship.path_pts:
            shifted_pts_list.append((point[0] + background_x_middle + (tile_width / 2),
